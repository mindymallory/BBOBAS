...
for (j in 1:length(dates)) {  
  ...
  for (i in 1:length(DeliveryDates)) {
    ...
    assign(paste0('q', '_', as.character(dates[j]), "_",as.character(DeliveryDates[i])) ,as.xts(subset(qtemp, select = -c(TradeDate, TradeTime)), order.by = times))
    save(paste0('q', '_', as.character(dates[j]), "_",as.character(DeliveryDates[i])), file = paste0('q', '_', as.character(dates[j]), "_",as.character(DeliveryDates[i]),".rda"))
    
    #The Trades
    ttemp <- subset(CUMULTRANS, SYMBOL == DeliveryDates[i])
    times <- timeDate(paste0(ttemp$TradeDate,ttemp$TradeTime), format = "%Y%m%d%H%M%S")
    assign(paste0('t', '_', as.character(dates[j]), "_",as.character(DeliveryDates[i])) ,as.xts(subset(ttemp, select = -c(TradeDate, TradeTime)), order.by = times))
    save(paste0('t', '_', as.character(dates[j]), "_",as.character(DeliveryDates[i])), file = paste0('t', '_', as.character(dates[j]), "_",as.character(DeliveryDates[i]),".rda"))
  }
    save(DeliveryDates, file = paste0(Contracts, as.character(dates[j])))
}
